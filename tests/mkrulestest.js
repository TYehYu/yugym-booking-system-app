...
console.log('  按之前看得到會發生什麼');
{
  ...
  /* 2026-09-11 使用者附截圖：「這是團課教練請假視窗　文字有點多？」—— 綠框段落收成四條短句，
     理由（補課券是會員自己請假的補償）搬進註解 */
  ok('★ 票券退回＋明講不發補課券（短句條列）',
     /<li>\$\{tkChip\('back','每個名額退回 1 堂'\)\}<\/li>/.test(F)
     && /<li><b>不發補課券<\/b><\/li>/.test(F)
     && /不發補課券，因為補課券是會員自己請假的補償/.test(F));
  ok('★ 沒有到期日的票照實顯示「無到期日」', /:'無到期日'/.test(F));
  ok('　　先提醒「找得到代課就改用代課」', /<li>找得到代課教練請改用「代課」<\/li>/.test(F));
