...
console.log('① 店長 → 主管');
/* 白名單：這兩處的「店長」是**資料相容**，不是畫面上的字。
   ・職稱下拉的舊稱選項（沒有它，舊資料一進編輯會被 fallback 改寫成「老闆」，0721 踩過）
   ・_MGR_TITLES：主管開關連動職稱時，要認得舊稱才正名得掉（2026-09-09） */
const _ALLOW=[`['店長','店長（舊稱）']`, `const _MGR_TITLES=['','主管','店長'];`];
const _bad=uiLines.filter(([,l])=>!_ALLOW.some(x=>l.includes(x)))
  .filter(([,l])=>/['"`>][^'"`]*店長/.test(l));
ok('★★★ 畫面上不再出現「店長」，只剩兩處資料相容用的舊稱',
   _bad.length===0, _bad.map(([n,l])=>n+': '+l.trim().slice(0,60)));
