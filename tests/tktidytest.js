...
fresh(); tkTidyPaint();
/* 2026-09-09 簡化：「佔堂數」改叫「已排堂數」（畫面上已經沒有勾選這個動作了）。 */
ok('★ 結算列寫出已排堂數與「現在 → 之後」', /已排堂數[\s\S]*?2 \/ 8/.test(S()) && /票面餘額[\s\S]*?0 → 6 堂/.test(S()));
...
/* 2026-09-09 簡化（使用者：「這邊的資訊有點複雜　你可以簡單化跟簡化視窗內容嗎」）——
   「這次會更動 N 堂的歸屬／課的歸屬尚未更動」那一行整條退場：0828 清單移除之後
   畫面上沒有任何地方能改歸屬，那一行永遠是「尚未更動」，是純噪音。 */
ok('★★ 歸屬那一行不再畫（畫面上已經改不了歸屬，永遠是同一句）',
   !/這次會更動/.test(S()) && !/課的歸屬尚未更動/.test(S()));
tkTidyTap('B4');
...
tkTidyPaint();
ok('★★ 超過總堂數：餘額畫成負數紅字並寫出多了幾堂',
   /ovd-neg/.test(S()) && /-1 堂/.test(S()) && /多出來的那 1 堂請先確認是不是該算在別張票上/.test(S()));
...
fresh(); tkTidyPaint();
/* 2026-09-09 簡化：帳目正常的票（絕大多數）那句固定不變的說明，
   從一整塊收成區塊標題旁的一句短的；舊票那段警告仍然整塊展開。 */
ok('★★ 帳目可信的票：說明收到標題旁，不再佔一整塊',
   NOTE()==='' && /扣課紀錄完整，建議「依戳記重算」/.test(boxes['tdy-rechint'].textContent||''));
