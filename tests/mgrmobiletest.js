...
/* 2026-09-09：訓練方案先只開放管理員，所以先用 wpEnabled 濾一次，再照 canTeach 濾行事曆。 */
ok('★★★ 側邊導覽也擋（原本只有底部導覽擋，從側邊還是進得去）',
   /function mobileCoachNavItems\(\)\{/.test(src)
   && /const base=MOBILE_COACH_NAV\.filter\(n=>n\.key!=='coach_plans'\|\|wpEnabled\(\)\);\s*\n\s*return canTeach \? base : base\.filter\(n=>n\.key!=='coach_calendar'\);/.test(src));
...
ok('★★ 底部導覽那一支跟著同一條線（2026-09-09 一起加了訓練方案）',
   /const base=COACH_BOTTOM_NAV\.filter\(n=>n\.key!=='coach_plans'\|\|wpEnabled\(\)\);\s*\n\s*return canTeach \? base : base\.filter\(n=>n\.key!=='coach_calendar'\);/.test(src));
