...
/* 0822 二修（使用者）：「多功能訓練架不用顯示」「8/22 9:00 也過期了 自主應該也不能預約」
   ⚠⚠ 2026-09-22 推翻「多功能不標」：三個場地都有空的時段只標得出另外兩個，
     使用者回報「訓練架被忽視了」「會造成客人誤會　整天都沒有訓練架可以用」。
     現在三個都有名字、三個都標；名稱統一成三個字（使用者：「還是統一改成
     訓練架　教室　跑步機」），與第二步確認視窗那三顆場地鈕（VOPT）同一套字。 */
t('三個場地都有名字，而且與確認視窗那三顆鈕同一套字',
  /VN=\{multi:'訓練架',group:'教室',treadmill:'跑步機'\}/.test(add)
  && /VOPT=\[\['multi','訓練架'\],\['group','教室'\],\['treadmill','跑步機'\]\]/.test(s));
/* 2026-09-22：一個時段可能三個場地都有空，改成「每個有空的場地各一列」
   （使用者：「該時段團課教室跟跑步機同時顯示　分成兩列」→「時間＋所有場地　所以下面要塞三列」）。
   ⚠ 沒有名字就不畫空 tag 這道防線留著：VN 少一個 key 時不要吐出空白列。 */
t('沒有名字的場地不畫空的 tag', /const nm=VN\[vid\]\|\|''; if\(!nm\) return '';/.test(add));
...
/* 2026-09-09 三修：「訓練紀錄」也要吃同一套外框（使用者回報頂欄與導覽列配色不一致）。
   ⚠ 這是白名單不是預設 —— 日後加會員頁要記得回來加一筆。 */
t('navTo 只在三個會員頁（我的預約／我的票券／訓練紀錄）且 memh2On 時掛 shell',
  /classList\.toggle\('memh2-shell', _mv\)/.test(s)
  && /\(key==='mem_bookings'\|\|key==='mem_tickets'\|\|key==='mem_training'\)\s*\n?\s*&& typeof memh2On==='function' && memh2On\(\)/.test(s));
...
console.log('\n快速預約視窗裡的日期列（換一天不必關窗）');
{
  const QS=cut('async function memh2SelfSlots(ds, until, reschedId){','function memh2SelSlot(');
  /* 2026-08-31：同一個視窗兼作改期 —— 標題會變、中間多一條「原時段」提示條，
     換日期時要把預約 id 帶著走（見 selfbarrestest.js）。 */
