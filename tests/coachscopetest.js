...
/* 0914：改時間與場地併進「更改」一顆（memEditAsk 再二選一），顯示條件一個字沒放寬 */
t('★★ 只有自主訓練能改時間', /\(selfServe && _isSelfBk && b\.member_id===SESSION\.id\)[\s\S]{0,180}'更改'/.test(src)
  && /<b>更改時間<\/b>/.test(src));
/* 2026-08-24 使用者指示：「會員端的自主訓練要新增一個圓形按鈕［更改時間］」——
   這一顆本來就在（0809 做的），只是字從「改時間」改成使用者說的「更改時間」。
   排除條件一條都不能少，所以在這裡逐條釘住。 */
t('　　教練課不給（時段要跟教練重新喬，櫃檯才看得到全部人的班表）',
  /教練課不給 —— 時段要跟教練重新喬/.test(src));
t('　　團課不給（名額制，要換時段是退掉這堂再報別堂）',
  /團課也不給：那是名額制/.test(src));
t('　　教練請假被改記成自主訓練的、以及場租，都不算自主訓練',
  /!\(typeof bkIsCoachLeave==='function' && bkIsCoachLeave\(b\)\)/.test(src)
  && /&& b\.category!=='場租';/.test(src));
t('★★ 團課的按鈕寫「取消名額」（不是整堂）',
  /st\.isGrp\?'取消名額':'取消'/.test(src));
/* 2026-08-24 使用者指示：「教練課就不要顯示取消的圓形按鈕了（會員端）」——
   同日稍早依 0821 定案補過一顆「洽櫃檯」，但那顆對會員沒有用處：按了只看到
   「請找櫃檯」，反而多一顆按不出結果的鈕。
   ⚠ 這一條一定要對著 memh2Tap 抓（會員實際走的那一支）—— 舊的 memTaskTap 裡
     還留著「洽櫃檯」，對整份原始碼抓會抓到它、變成假通過。 */
{
  /* ⚠ 先剝掉註解 —— 決定的理由本來就寫著「洽櫃檯」三個字，不剝會被自己的說明誤判。 */
  const h2=src.slice(src.indexOf('async function memh2Tap(id){'), src.indexOf('async function memh2SelfSlots('))
    .replace(/\/\*[\s\S]*?\*\//g,'');
  t('★★ 會員端的教練課不畫取消圓鈕（連「洽櫃檯」都不要）',
    !/洽櫃檯/.test(h2)
    && /let cxBtn=selfServe/.test(h2));
  t('　　已上完／已過時的自助課仍留「不可取消」（那是狀態說明，不是入口）',
    /\(\(st\.isGrp \|\| _isSelfBk\) && \(st\.done\|\|st\.past\)\) \? orb\('off','✕','不可取消',null\) : ''/.test(h2));
}
