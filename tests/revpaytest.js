...
console.log('① 資料來源');
ok('★ 票券列：同日同票券的收款反查（含 purchase id，修正要用）',
   /if\(p\.ticket_id&&puLocalDate\(p\)===date\) _payByTk\[p\.ticket_id\]=\{m:p\.payment_method\|\|'',pid:p\.id\};/.test(src));
ok('★ 票券列帶 pay＋payRef', /pay:_payLb\(\(_payByTk\[t\.id\]\|\|\{\}\)\.m\), payRef:\(_payByTk\[t\.id\]\|\|\{\}\)\.pid\|\|null,/.test(src));
ok('★ 收款列直接帶', /pay:_payLb\(p\.payment_method\), payRef:p\.id,/.test(src));
...
ok('★ 付款方式疊在金額上方（2026-08-03 使用者指示：並排會把品項擠掉）',
   (src.match(/<span class="mc-rev-r">\$\{revUndoChip\(r\)\}\$\{revPayChip\(r\)\}<span class="mc-rev-amt">/g)||[]).length===2
...
ok('★ 修正視窗：現金／匯款二選一（不列刷卡）、講清楚影響財務報表',
   ...
   && /\$\{btn\('cash'\)\}\$\{btn\('transfer'\)\}<\/div>/.test(src)
   && !/\$\{btn\('card'\)\}/.test(src));
