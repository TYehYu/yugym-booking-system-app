...
console.log('\n③ 補扣視窗：超出票券堂數的待簽約卡位');
{
  ...
  ok('★★ 會被刪除的日期要列出來（按下去之前先看到）',
     /* 2026-09-11：會刪東西的那一件獨立成紅字一條，日期放在同一條的小字 */
     /<div class="mk-key">超出堂數的 \$\{over\.length\} 堂待簽約會一併刪除<small>\$\{over\.slice\(0,6\)\.map\(_od\)\.join\('、'\)\}\$\{over\.length>6\?`…等 \$\{over\.length\} 堂`:''\}<\/small><\/div>/.test(src));
