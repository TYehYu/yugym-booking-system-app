...
console.log('\n算進本月利潤（2026-07-31 使用者定案）');
{
  ...
  /* 2026-08-23 使用者指示：利潤下方那排說明文字整行移除（每次都佔兩行）。
     「有沒有登錄支出」改由列表裡新的「支出」那一列表達：金額 $0 時副標寫
     「尚未登錄，點這裡新增」，而且整列點下去就能補登 —— 比一句說明更有用。 */
  ok('★ 沒登錄支出時講清楚（不是默默當成 0），而且點得進去補登',
     /ovRow\(OV_IC\.exp,'支出',fmtNT\(otherExp\),'',otherExp>0\?'':'尚未登錄，點這裡新增',\{red:true,tap:`openExpensePick\('\$\{ym\}'\)`\}\)/.test(dash));
  ok('　　說明文字那一行已移除（算法沒變，只是不再每天複述）',
     !/與經營報表損益表同一套算法（含營業稅、公司負擔勞健保/.test(src)
     && /那句話（與經營報表同一套算法…）只在第一次看的時候有用/.test(src));
