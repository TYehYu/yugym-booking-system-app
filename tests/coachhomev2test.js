...
/* 0822 二修（使用者）：「教練會建立教練課，所以跑步機的選項就不用進來了，
   ...
   都沒了就不要顯示」 */
/* 2026-09-03 更名：多功能訓練架 → 史密斯訓練架（縮寫仍是「訓練架」） */
ok('★★ 教練課只用兩種場地：不標＝史密斯訓練架、標「團課教室」＝訓練架滿了',
   /return \{ok:true, tag: vid==='group'\?'團課教室':''\};/.test(src)
   /* 0822：說明文字改成條列（使用者：「處理一下上方的斷句」），字句跟著拆開 */
   && /<li>沒標的就是史密斯訓練架<\/li>/.test(src)
   && /<li>標「團課教室」＝訓練架已滿，那一堂會上在教室<\/li>/.test(src));
