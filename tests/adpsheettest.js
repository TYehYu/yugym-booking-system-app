...
console.log('① 兩欄');
{
  ...
  eq('★★★ 清單順序就是使用者指定的分欄（先填左欄三個，再填右欄三個）',
     L.map(c=>c.name), ['教練課','團體課','運動按摩','場地租借','團課體驗','自訂方案']);
...
/* 2026-09-22：使用者講清楚「自主訓練我們沒有單獨販售，但是預約的時候可以幫客戶
   預約自主訓練，現在要新增預約場地租借」——
   ・銷售那張卡改回「場地租借」（票種也換回 tt-venue-rental，見 index 的說明）
   ・「自訂」改成「自訂方案」
   ・建立預約與更換課程都放回「場地租借」
   ⚠ 0729「場租＝自主訓練，不重複列」與 0801「場地租借改名自主訓練」兩個決定都被推翻，
     因為前提是錯的：場租不計堂數、有自己的票種，自主訓練扣會員的點。 */
ok('★★ 自訂方案仍然進得去（「選擇課程」那張視窗裡本來就有一項）',
   /\{k:'custom',   name:'自訂方案',/.test(src));
/* 2026-09-22 二修（使用者：「這邊場地租借　副標是單次·效期7天　是不是也要修改」）——
   同一天已經改成可以賣多堂數的方案，這張卡卻還寫著寫死的「單次 · 效期 7 天」。
   ⚠ 幾堂、多少錢、效期多久一律留給**下一步的方案下拉**講 —— 那裡才知道實際是哪一個。 */
ok('★★★ 場地租借的副標不寫死堂數與效期',
   /\{k:'facility', name:'場地租借', sub:'選方案 → 發場租票'/.test(src)
   && !/sub:'單次 · 效期 7 天'/.test(src));
ok('★★ 刻意不寫「→ 業績歸屬」（場租不歸教練業績、也不計堂數）',
   /這裡刻意不寫「→ 業績歸屬」：場租不歸教練業績、也不計堂數/.test(src));
