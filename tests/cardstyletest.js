...
ok('★ ① 時間＋姓名靠左、整組垂直置中（兩張卡成對）',
   /\.cal-ev\.cal-ev-std \.evc-txt,\s*\n\s*\.tcard\.tcard-std \.tcard-txt\{ justify-content:center !important; \}/.test(blk));
...
ok('★ ③ 出席章改成姓名右邊的行內小圓標（不再是 absolute 角標）',
   /\.cal-ev\.cal-ev-std \.evc-check,\s*\n\s*\.tcard\.tcard-std \.tcard-chk\{\s*\n\s*position:static !important;/.test(blk)
   && /border-radius:999px !important;/.test(blk));
ok('　　姓名列是 flex，名字可縮、章不縮',
   /\.evc-nmrow,\s*\n\s*\.tcard\.tcard-std \.tcard-nmrow\{ display:flex; align-items:center;/.test(blk)
   && /min-width:0; flex:0 1 auto;/.test(blk));
...
ok('★ 首頁：章也進了姓名列',
   /<span class="tcard-nmrow"><span class="tcard-mem">\$\{nm\}<\/span>/.test(src));
