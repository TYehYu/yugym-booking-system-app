...
/* 2026-09-04：這一條拆成兩半。行事曆課卡改成四列（章左上／時間右上／姓名／場地／教練），
   教練不再推到右下角；首頁課卡（.tcard）沒有要求改，維持原本的右下角。 */
ok('★ ② 首頁課卡的教練仍在右下角（margin-top:auto 推到底＋align-self 靠右）',
...
ok('★ 行事曆：章排在姓名之後、自成一列，外層不再重複輸出',
   /<span class="evc-nmrow"><span class="evc-name\$\{bkNameBlankCls\(b\)\}\$\{_nmVCls\}">\$\{_stdName\}<\/span>\$\{_stampOut\}<\/span>\$\{_stdTag\}/.test(src)
...
ok('★ 「更改場地」→「更換場地」（與更換票券同一組動詞）',
   /* 2026-09-11：標題維持「更換場地」，副標改寫「能做什麼」（自主訓練多一個跑步機人數） */
   /,'更換場地','更改場地・更改跑步機人數'\);/.test(src) && /,'更換場地','更改場地'\);/.test(src)
   && !/row\([^)]*,'更改場地',\(typeof venueDisplay/.test(src));
...
ok('　　桌機是左上角小圓章（0904 從左下搬上來），手機的角標基底規則不動',
