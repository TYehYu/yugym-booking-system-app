...
const helpers=new Function('COURSE_SHAPE','parseYmd','attObj','ymd','TODAY',[grabFn('tkVisual'),grabFn('bkSelfBooked'),grabFn('tkDoneMarker'),grabFn('ticketTokens'),grabFn('mids'),
...
(async()=>{
  ...
  console.log('\n團課票券可指定要扣哪一張');
  {
    ...
    /* 四處：兩個視窗各一次，加上 0829 ［＋新增］的單選與「清掉選擇」（換人／清空
       時都要一併丟掉舊的票券指定，否則會殘留到下一位身上）。 */
    chk('　　每次開視窗重置指定，不跨堂殘留',
      (src.match(/window\._grpTkPick=\{\};/g)||[]).length===4);
