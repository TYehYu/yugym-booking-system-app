...
console.log('欄位順序與分區');
{
  ...
  /* 2026-08-08 使用者指示（附截圖）：「票券種類中間也新增那個格線分開」——
     原本只有中區起點（教練課）與右區起點（最近上課）有線，五個課別之間靠留白區分；
     圓點一多就摺行，摺下來那一列看起來像下一格的東西。改成每一個課別都給線。 */
  eq('★★ 每個票券種類之間都有分區線，右區起點（最近上課）照舊',
     cols.filter(c=>c.zone).map(c=>c.label),
     ['教練課','團體課','自主訓練','運動按摩','折價券','最近上課']);
  eq('　　連動核對那欄也維持有線', mkCols(true).filter(c=>c.zone).map(c=>c.label),
     ['教練課','團體課','自主訓練','運動按摩','折價券','最近上課','連動核對']);
...
/* 2026-09-07：改成帶字的「另 N 張」—— 原本的「＋N」跟圓點裡的「+19（還有幾堂）」
   長得幾乎一樣，同一格裡兩種意思分不出來。 */
ok('★ 同課別還有幾張在用會標出來', /另 \$\{n-1\} 張<\/span>/.test(src) && /\.tkcat-n\{font-size:11px;/.test(src));
ok('★ 而且不會再跟圓點裡的「+堂數」撞號', !/＋\$\{n-1\}/.test(src));
