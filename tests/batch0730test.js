...
/* 2026-09-11：教練端取消視窗收成「24 小時內紅字一條」（共用 .mk-key） */
ok('★ 24 小時內用紅字標明是臨時取消',
   /\? `<div class="mk-key">距離開課不到 24 小時<small>臨時取消：目前仍會退回票券/.test(src));
ok('★ 修掉「畫面說不退、程式卻退」的矛盾（auto 目前一律退課）',
   /臨時取消：目前仍會退回票券，請先向會員說明；要扣課請找櫃檯/.test(src)
   && !/⚠ 距離開課不到 24 小時，取消後<b>視同使用、不退回票券<\/b>。/.test(src));
/* 2026-09-11：教練端原本抄了一份贈點警告，改成兩張取消視窗都呼叫同一支 */
ok('★ 贈點會被收回也先講（教練端與櫃檯端都有，共用同一支）',
   /async function cancelRewardWarnHtml\(b\)\{/.test(src)
   && /\$\{await cancelRewardWarnHtml\(b\)\}/.test(src)
   && /try\{ _rwLine=await cancelRewardWarnHtml\(b\); \}catch\(_\)\{\}/.test(src)
   && /取消後收回 \$\{pt\} 點自主訓練贈點/.test(src));
