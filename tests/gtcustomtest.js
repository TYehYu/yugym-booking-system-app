...
console.log('\n③ 清單本來就有兩項，不用新增');
/* 2026-09-22：自主訓練從白名單拿掉（使用者：「而且自主訓練本來就沒有在販售」）——
   它會在裡面是 0801「場地租借改名自主訓練」那次的殘留。0922 場地租借已經有自己的卡，
   這一項只剩下害人：櫃檯當天就是從這裡賣出一張 $200 的「自主訓練」票，
   結果掉進今日營收的縫裡（見 tests/revselftest.js）。
   ⚠ 查過才拿掉：自主訓練票 1,184 張裡 1,108 張是簽到贈送，
     76 張 source='purchase' 但金額合計只有 $400（74 張是 $0 的發放）。
   ⚠ 發放贈送點數不受影響 —— 那條走「票券模板」（course_plans），不吃這份白名單。 */
ok('★★ _CUSTOM_TT 已含「教練課」與「友善教練課」',
   /const _CUSTOM_TT=\['教練課','友善教練課','團體課'\];/.test(src));
ok('★★★ 自主訓練不在自訂銷售的白名單裡（本來就沒在賣）',
   !/_CUSTOM_TT=\[[^\]]*自主訓練/.test(src)
   && /而且自主訓練本來就沒有在販售/.test(src));
ok('★★ 拿掉的理由與查證數字寫在原地',
   /1,108 張是簽到贈送、76 張 source='purchase'/.test(src)
   && /\*\*不影響發放贈送點數\*\*/.test(src));
