...
/* 2026-09-15 使用者：「我發現當天的收款順序沒有按照收費順序　幫我排序」——
   兩段收款列（票券／其他收款）先合併成 _revPayRows 依 created_at 升冪排，
   抽獎不參與那次排序、仍舊接在後面。
   ⚠ 這一條同時守著「抽獎在最後」與「排序只作用在收款列」：
     要是哪天有人把 _dayLot 併進 _revPayRows 一起排，抽獎就會混進收款裡。 */
ok('★★ 抽獎排在收款之後（這一頁叫「今日營收」，錢要先看到）',
   /const _revRows=\[\.\.\._revPayRows,\s*\n\s*\/\* 抽獎排在最後/.test(src)
   && /_revPayRows\.sort\(\(a,b\)=>\{/.test(src)
   /* 沒有 created_at 的列要墊底 —— 空字串在 localeCompare 下會搶到第一名 */
   && /if\(!_a\) return 1;/.test(src));
