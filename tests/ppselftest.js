...
/* 2026-09-14 二修：載具與 Email 搬到頂列姓名區（.pp-idinv），收合也看得到；
   會員本人一樣看得到、改得動（權限沒動，仍是 canEditMemberData()||isSelf）。 */
t('緊急聯絡人／LINE 照舊，載具與 Email 移到頂列',
  /ecItem \+ lineItem \+ carrierItem/.test(hd)
  && /<div class="pp-meta pp-idinv">\$\{carrierItem\}\$\{emailItem\}<\/div>/.test(hd));
...
/* 0914：入口收成一顆「更改」，條件不變 */
t('★ 只有自主訓練能改時間', /\(selfServe && _isSelfBk && b\.member_id===SESSION\.id\)[\s\S]{0,140}'更改'/.test(s));
t('★ 只有團課與自主訓練能自行取消',
...
t('★ 教練課只有簽到（更改不給，取消圓鈕整顆不畫）',
  /let edBtn=\(selfServe && _isSelfBk && b\.member_id===SESSION\.id\)/.test(s)
