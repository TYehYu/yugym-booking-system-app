...
ok('★ 掛在表頭右上（與核對按鈕並排）', /\(isM \? lvBtn\+delBtn :/.test(src));
{
  ...
  ok('★★ 事前講清楚：不能復原、會連登入帳號一起刪',
     /* 2026-09-11：「不能復原」獨立成紅字一條，其餘條列 */
     /<div class="mk-key">不能復原<\/div>/.test(F)
     && /<li>給<b>註冊錯誤／輸入錯誤<\/b>的資料用<\/li>/.test(F)
     && /<li>連<b>登入帳號<\/b>一起刪，他才能重新用 LINE 申請<\/li>/.test(F));
...
}
{
  ...
  ok('★★ 並且告訴櫃檯那種情況該怎麼辦（不是只說不行）',
     /* 2026-09-11：「刪不掉」那張也收成條列 */
     /<li>買錯／打錯的票券 → 30 分鐘內可在首頁今日營收整筆退回，超過就走退費<\/li>/.test(F)
     && /<li>排錯的課 → 先取消預約<\/li>/.test(F)
     && /<li>只是資料填錯 → 直接改就好，不必刪除<\/li>/.test(F));
  ok('★ 各種錯誤碼都有對應的話',
