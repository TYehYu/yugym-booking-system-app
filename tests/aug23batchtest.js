...
/* 2026-09-02：出席章 16→22px，左內距再從 10 收到 8 討回 2px。
   2026-09-03：欄距再從 6 收到 5 —— 同日稍早把續約徽章改成跟時間並排，
   多吃掉 18px，把場地擠成「史密斯訓…」。完整那筆帳與另外三項調整
   見 tests/tcardwidthtest.js，這裡只守內距／欄距這一項。 */
ok('　　內距與欄距一起收（10\\/9\\/10\\/11→9\\/7\\/9\\/10、7→6；0902 左內距 8；0903 欄距 5）',
   /gap:0 5px;align-items:center;padding:9px 7px 9px 8px;\}/.test(src));
/* 場地那一列能不能放得下，0903 起改用實測（Playwright 量真實 CSS），
   不再靠註解裡的手算 —— 手算那筆正是因為沒跟上徽章改動才失準的。 */
...
ok('★★ 「回到今天」固定在左邊，不再分左右',
   ...
   /* 只看桌機那條日期列 —— 手機版（.mtc-wrap-top）另有一顆，不在這條規則裡。
      0902 二修後那一格永遠有東西：不是「回到今天」鈕，就是「今天」狀態。 */
