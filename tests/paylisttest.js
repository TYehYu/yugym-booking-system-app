...
/* 2026-09-22：那三行說明收掉了（使用者問「這邊有優化建議嗎」）——
   與 0901 對「即將降級名單」做的是同一件事：櫃檯每天看，規則學一次就好，
   每次開窗先讀三行反而把名單推到摺線下（4 位就要捲動了）。
   ⚠ 這一條改成守「真的收乾淨、也寫下為什麼」，不是守那句話還在。 */
ok('★ 說明那三行已移除，理由寫在原地', !/今天要向這些人收款/.test(src)
   && /說明那三行收掉（2026-09-22）/.test(src));
...
ok('★ 名字旁邊標上課時間', /const tm=\(it\)=>it\.time\?`<span class="tdl-tm\$\{it\.pay\?' tdl-tm-pay':''\}">\$\{it\.time\}<\/span>`:'';/.test(src)
   ...
   /* 2026-09-22：名字列多一個金額章（只有分期算得出來），排在時間之後、狀態之前 */
   && /\$\{it\.name\|\|'—'\}\$\{co\(it\)\}\$\{tm\(it\)\}\$\{amt\(it\)\}\$\{tag\(it\)\}/.test(src));
