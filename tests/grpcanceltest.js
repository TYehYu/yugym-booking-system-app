...
ok('★ 團課改看扣課帳本，不再只看 bookings.ticket_id',
   /const noTicket = \(\(!b\.ticket_id && _grpNetDeduct<=0\) \|\| _neverDeducted\) && !_netUnknown;/.test(src));
/* 2026-09-11：改問資料庫（bkNetDeductDB 的算法同樣是 deduct − refund）；問不到不准判成沒扣過 */
ok('★ 淨扣課＝deduct 筆數 − refund 筆數（重覆取消不會重覆問）',
   /const _n=await bkNetDeductDB\(b\.id, null\);\s*\n\s*if\(_n===null\) _netUnknown=true; else _grpNetDeduct=_n;/.test(src));
...
/* 2026-09-11：bkNetDeductDB 自己吞錯回 null（不會丟例外），null 走 _netUnknown＝讓人選退回／扣課 */
ok('　　讀不到帳本不能讓取消視窗開不起來（null → 讓人選，不判成沒扣過）',
   /⚠ 問不到（null）＝ _netUnknown：不准走「沒扣過」那張/.test(src));
