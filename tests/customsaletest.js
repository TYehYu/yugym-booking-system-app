...
ok('★★ 沒有任何場租方案時，告訴櫃檯去哪裡建',
   /* ⚠⚠ 2026-09-22：這個路徑寫錯過兩次（「方案管理」與「票券管理 → 票券模板」都不存在，
      後者的分頁早就從 TK_TABS 移走了）。使用者回報「只看到單次 沒看到10次的」，
      因為他照著錯的路徑找不到地方建。真的那一個是 系統設定 → 課程方案。
      ⚠ 這一條同時守「提示存在」與「指到真的有的頁面」—— 寫路徑之前先 grep 導覽標籤。 */
   /先到<b>系統設定 → 課程方案<\/b>，在「票券模板」按〔＋ 新增模板〕/.test(src)
   && /\{grp:'環境設定', label:'課程方案', page:'settings_ticket'\}/.test(src)
   && /PAGES\.settings_ticket=async function\(\)\{/.test(src));
