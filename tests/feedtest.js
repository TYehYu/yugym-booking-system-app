...
ok('　　同一則不會插兩張（左右兩區一起找）',
   /document\.querySelector\(`\.dfeed-card\[data-nid="\$\{n\.id\}"\]`\)\) return;/.test(src));
...
/* 2026-08-01 使用者指示：右下角的卡改成「點卡片＝移動到那堂課」，確認鈕移除；
   只有讀不出日期、沒有地方可去的那種才保留確認鈕（否則收不掉）。 */
ok('　　讀不出日期的那種仍保留確認鈕（勾號）', /✓ 確認<\/button>/.test(src));
