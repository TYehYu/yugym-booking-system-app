...
console.log('\n④ 總堂數的定義');
/* 2026-08-26 使用者：「應該不要算進去　不然教練會誤會他有達標獎金」「體驗就單獨紀錄」
   → 體驗退出總堂數（欄位照舊獨立列）。這一支原本驗的是 0808 那版「含體驗」的說法。 */
ok('　　改成「算堂數的課」＝教練課＋團課＋自主陪同（體驗退出）',
   /總堂數（2026-07-31 使用者指示）＝這個月實際帶的、\*\*算堂數\*\*的課/.test(src)
   && /（教練課＋團體課＋自主訓練陪同）/.test(src));
ok('　　體驗仍然自己一欄（使用者指定「單獨紀錄」）',
   /trial:done\.filter\(isTrial\)\.length, trialAll:mine\.filter\(isTrial\)\.length,/.test(src));
