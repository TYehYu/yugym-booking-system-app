...
ok('★ 三個區塊各自涵蓋正確的欄位範圍（姓名 1–3｜表現 4–11｜權限 12–13）',
   /\.st-hz-1\{grid-column:1 \/ 4;text-align:left;\}/.test(src)
   && /\.st-hz-2\{grid-column:4 \/ 12;\}/.test(src)
   && /\.st-hz-3\{grid-column:12 \/ 14;text-align:left;\}/.test(src));
...
ok('★ 資料列的分區線位置與表頭一致（總堂數＋權限開關）',
   /num\(st\.all, [^)]*, 0, 'st-zb'\)/.test(src)
   && /<span class="st-l-sw st-zb" onclick="event\.stopPropagation\(\);">/.test(src));
ok('　　區塊標題本身垂直置中，不被拉伸的分隔線帶歪', /\.st-lhead \.st-hz\.st-zb\{align-self:center;\}/.test(src));
...
ok('★ 權限開關欄撐滿自己的格子（分區線才畫得出來）', /\.st-l-sw\{justify-self:stretch;\}/.test(src)
