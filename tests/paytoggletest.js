...
console.log('① 開關出現在員工列表那一排');
{
  ...
  /* 2026-08-02 使用者回報（附截圖）：「按鈕跟齒輪重疊到了，把按鈕裡面的燈號取消，不要佔位子」
     —— 七顆排不下，最後一顆壓到齒輪。燈號在列表裡是多餘的：開／關靠底色與框線就分得出來。 */
  ok('★ 列表裡的開關不畫燈號（七顆才排得下，不會壓到齒輪）',
     /\.st-l-sw \.st-swb \.st-swd\{display:none;\}/.test(src));
  ok('　　窄螢幕也沒有把燈號縮小後又長回來的規則',
     !/\.st-l-sw \.st-swb \.st-swd\{width:/.test(src));
  ok('　　員工卡（直式）的燈號保留，空間夠', /\.st-swb \.st-swd\{width:14px;height:4px;/.test(src));
  ok('　　為什麼拿掉燈號，寫在程式裡',
     /那一條小橫線只是重複同一件事，卻每顆吃掉 17px。/.test(src));
