...
/* 2026-09-15 使用者：「有效票券的格局跟歷史紀錄票券的格局分佈不同嗎　有辦法整理一致嗎」——
   histRow（歷史／過期卡）改成照 cardOf 的版型走：櫃檯以上看到的是
   「購買・效期提到圓圈上方」，其他角色維持原本的單行底列。
   兩個分支各出現一次 tkBuyDateHtml，所以這一張卡從 1 處變 2 處，總數 5 → 6。
   （cardOf 本來就是這個模式，現在兩張卡完全對稱。） */
ok('★ 票券卡都列出來（會員名片可用／歷史、管理端、會員詳細、預約明細）',
   (src.match(/\$\{tkBuyDateHtml\(t\)\}/g)||[]).length===6
