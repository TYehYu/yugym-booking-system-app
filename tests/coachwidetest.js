...
ok('★ 上方導覽顯示、底部導覽收起',
   ...
   && /#app-screen\.coach-wide\.role-coach #bottom-nav\{display:none !important;\}/.test(src));
ok('★ sticky 高度改成含導覽列（62 → 118）', /#app-screen\.coach-wide\.role-coach\{--fixed-h:118px;\}/.test(src));
...
console.log('\n導覽項目');
ok('★ 教練桌機只放 首頁 ＋ 預約行事曆',
   /items=\[\{key:'coach_today',label:'首頁'\},\{key:'calendar',label:'預約行事曆'\}\];/.test(src));
ok('★ 只在「非手機且非店長」時套用（店長仍走 buildNav2 側欄版）',
   /if\(!isMobile && !SESSION\.is_manager\)\{[\s\S]{0,400}items=\[\{key:'coach_today',label:'首頁'\}/.test(src));
