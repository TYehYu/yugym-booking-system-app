...
eq('★★ 欄序：教練課／團體課／值班／達標／續約／管理職／生日／請假扣薪／應發／勞保／健保／實領／公司負擔',
   COLS.map(c=>c.t),
   ['教練課','團體課','值班','達標獎金','續約獎金','管理職','生日禮金','請假扣薪',
    '應發合計','勞保','健保','實領','公司負擔']);
eq('★★ 加項／減項／合計分得出來（畫面靠這個上色）',
   COLS.map(c=>c.kind),
   ['add','add','add','add','add','add','add','cut','sum','cut','cut','sum','co']);
ok('★★ 每一欄都有一句說明（就是使用者要的「標示每一個金額代表什麼」）',
   COLS.filter(c=>c.k!=='bday').every(c=>!!c.s));
ok('★ 勞保／健保標明是「員工自付」',
   COLS.find(c=>c.k==='labor').s==='員工自付' && COLS.find(c=>c.k==='health').s==='員工自付');
ok('★★ 公司負擔那欄標明含勞退與職災',
...
console.log('\n③ 表怎麼畫');
{
  ...
  ok('★★ 表下方說清楚「勞保健保不是公司的額外成本」',
     /<b>勞保／健保<\/b>是員工自付、從應發裡代扣，<b>不是<\/b>公司的額外成本；/.test(F));
  ok('★ 並且算給你看公司實際的人事支出',
     /公司這個月實際的人事支出＝應發合計 \$\{m\(tot\.gross\)\} ＋ 公司負擔 \$\{m\(tot\.co\)\} ＝ <b>\$\{m\(tot\.gross\+tot\.co\)\}<\/b>。/.test(F));
