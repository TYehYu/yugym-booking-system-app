...
console.log('① 規則本身（validateBooking 的限時段檢查）');
{
  ...
  /* 2026-08-08 使用者更正：「平日 17:30 也是不能預約友善教練課的時間」——
     比的改成下課時間（ne＝開始＋時長），17:30 的 60 分鐘課會上到 18:30。 */
  const seg=/const dow=parseYmd\(date\)\.getDay\(\);[\s\S]*?此票券僅限平日使用（假日不可預約）';/.exec(grabFn('validateBooking'))[0];
  const chk=new Function('date','ns','ne','dur','parseYmd','minToTime',
    'return (function(){'+seg+' return null; })();');
  const P=x=>new Date(x+'T00:00:00');
  const M=m=>String(Math.floor(m/60)).padStart(2,'0')+':'+String(m%60).padStart(2,'0');
  const C=(d,st,dur)=>chk(d,st,st+(dur||60),dur||60,P,M);
  eq('★ 平日 10:00（60 分）→ 可以', C('2026-08-10',600), null);
  eq('★★ 平日 17:00（60 分，18:00 下課）→ 可以（貼著界線）', C('2026-08-10',1020), null);
  eq('★★ 平日 17:30（60 分，18:30 下課）→ 不行',
     C('2026-08-10',1050), '此票券僅限平日 18:00 前上完（60 分鐘的課最晚 17:00 開始）');
  eq('★★ 平日 19:30 → 不行（使用者最初回報的那一種）',
     C('2026-08-10',1170), '此票券僅限平日 18:00 前上完（60 分鐘的課最晚 17:00 開始）');
  eq('★★ 120 分鐘的課最晚 16:00 開始', C('2026-08-10',960,120), null);
  eq('　　120 分鐘的課 16:30 → 不行（18:30 下課）',
     C('2026-08-10',990,120), '此票券僅限平日 18:00 前上完（120 分鐘的課最晚 16:00 開始）');
  eq('★★ 週六 10:00 → 不行（假日一律不行，時間再早也一樣）',
     C('2026-08-08',600), '此票券僅限平日使用（假日不可預約）');
  eq('★ 週日 10:00 → 不行', C('2026-08-09',600), '此票券僅限平日使用（假日不可預約）');
  eq('　　早上 8:00 可以（不設下限；2026-07-20 使用者更正）', C('2026-08-10',480), null);
}
...
console.log('\n③ 建立之前就先講規則');
{
  ...
  ok('★ 改之前先查過正式庫，確認不會讓既有預約變成違規',
     /既有的友善課最晚就是 17:00 開始（18:00 下課，98 筆），/.test(src));
  ok('　　為什麼要先講（櫃檯是在排連續預約）',
...
console.log('\n④ 轉正時說得出真正的原因');
{
  ...
  ok('★★ 沒有可用票券時先分辨「是沒票，還是時段不合」',
     ...
     && /const _bad=!\(_dow>=1&&_dow<=5\) \|\| \(timeToMin\(b\.start_time\)\+\(Number\(b\.duration\)\|\|60\)\)>1080;/.test(F));
  ok('★★ 訊息指出是哪一堂、該怎麼辦（不是叫櫃檯去找一張已經在手上的票）',
     /僅限平日、且要在 18:00 前上完 —— 這堂是 \$\{b\.date\.slice\(5\)\.replace\('-','\/'\)\} \$\{b\.start_time\}，請先改時段再轉正/.test(F));
