...
ok('7/27 10:00 真實情境：多功能＋教室皆滿 → 分跑步機', rA.unit==='treadmill_1'&&rA.overflow);
ok('指定多功能 → 額滿', !!lib.allocateVenue('自主訓練',A,600,660,null,'multi').error);
ok('指定教室 → 額滿', !!lib.allocateVenue('自主訓練',A,600,660,null,'group').error);
ok('指定跑步機 → 可約', lib.allocateVenue('自主訓練',A,600,660,null,'treadmill').unit==='treadmill_1');
...
const D=[...A,mk('e','自主訓練','10:00'),mk('f','自主訓練','10:00')];
ok('六位全滿 → 額滿', !!lib.allocateVenue('自主訓練',D,600,660,null).error);
