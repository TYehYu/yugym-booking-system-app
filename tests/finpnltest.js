...
/* 2026-08-08 使用者提問「人事成本還沒扣掉勞健保嗎」→ 人事成本納入公司負擔勞健保 */
ok('★ 淨利＝銷課 − 稅 − 人事（薪資＋公司負擔勞健保）− 固定 − 其他',
   /const staffCost=salary\+coIns;/.test(F)
...
ok('★★ 公司負擔勞健保現在計入人事成本，並在表上單獨一列',
   /\$\{coIns>0\?sub\('公司負擔勞健保',coIns,'勞保雇主＋健保雇主＋勞退'\):''\}/.test(F)
   && /＋ <b>公司負擔勞健保<\/b> \$\{m\(coIns\)\}（勞保雇主＋健保雇主＋勞退）/.test(F));
ok('★ 明說員工自付的那一份不必再扣（已含在應發裡）',
   /員工自付的勞健保不必另外扣 —— 那是從應發裡代扣的，已經含在應發裡。/.test(F));
