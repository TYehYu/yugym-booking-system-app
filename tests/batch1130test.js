...
console.log('\n管理員次選單分組');
{
  ...
  /* 2026-09-09 加了「訓練方案」（管理員入口，接在動作資料庫旁邊） */
  ...
  ok('★ 環境設定：課程方案／場地・班別／合約範本／通知範本／動作資料庫／訓練方案',
     items.slice(5).join()==='課程方案,場地・班別,合約範本,通知範本,動作資料庫,訓練方案', items.slice(5));
  /* 訓練方案緊接在動作資料庫後面 —— 動作是材料、方案是菜單，同一組東西 */
  ok('　　訓練方案接在動作資料庫旁邊', items.slice(-2).join()==='動作資料庫,訓練方案', items.slice(-2));
