...
console.log('\n管理員次選單分組');
{
  ...
  /* 2026-08-08：票券管理入口移除（內容併進經營報表）→ 10 → 9 個項目
     2026-08-23：環境設定新增「通知範本」（LINE 自動通知的文字，管理員自行調整）→ 10 項
     2026-09-12：動作資料庫整頁退場（統一用每位教練自己的「常用動作」）→ 9 項 */
  ...
  ok('★ 環境設定：課程方案／場地・班別／合約範本／通知範本',
     items.slice(5).join()==='課程方案,場地・班別,合約範本,通知範本', items.slice(5));
  /* 2026-09-09：訓練方案曾短暫放在這一組，同日依使用者指示提到頂欄自己一組（g_train）。
     2026-09-12：動作資料庫也走了 —— 使用者選「停用，統一用常用動作」，
     設定入口改成訓練方案頁裡的「我的常用動作」卡片。 */
  ok('　　訓練方案不在這一組（它在頂欄自己一組）', items.indexOf('訓練方案')<0);
...
ok('★ 銷課堂數改成接在「銷課金額」標題後面（不再自成一列）',
   /after:`\$\{doneMainCount\} 堂`/.test(src));
ok('★ 組成仍算得出來，改寫在明細視窗裡（回答「726 怎麼算的」）',
   /const _doneMix=\(\(\)=>\{/.test(src) && /\$\{o\.mix\?`<br>組成：\$\{o\.mix\}`:''\}/.test(src));
