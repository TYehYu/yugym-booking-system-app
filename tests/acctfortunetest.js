...
/* 2026-09-08 多一條「更換授課教練」，也掛 !_leave（請假中的課不該換主責） */
/* 2026-09-21：改數 `!_leave`（不限定後面接 &&）—— 那次把「被隱藏的選項」改成
   淡化列出，多了三個分支，其中兩個是 `else if(!_leave)` 沒有 &&，
   用舊寫法會漏數而假性失敗。這條守的語意沒變：請假中的堂，除了復原以外一律不給改。
   ⚠ 在這張清單新增一列時，這個數字要跟著加 —— 那正是它存在的目的（強迫你想一下
     新那一列在教練請假時該不該出現）。 */
ok('　　已請假的堂，其他修改一律不列（每一條都掛 !_leave）', (ei.match(/!_leave/g)||[]).length===13,
