...
console.log('① 矩陣算得對（實跑 finMatrix，假 DB＋假 DOM）');
{
  ...
  (async()=>{
    ...
    /* 2026-08-31 二修（使用者：「只要列出 產品＋金額就好　其他文字太多餘」）——
       ...
       後者就是滑鼠底下那格的數字本身。 */
    ok('★★★ 團課收入那格＝產品＋金額，沒有別的字',
       /title="團體課　\$5,000"/.test(out), (out.match(/title="[^"]*5,000[^"]*"/g)||[]));
    ok('★★★ 其他那格＝產品＋金額（對不到票券的收款退回「收款」）',
       /title="收款　\$100"/.test(out), (out.match(/title="[^"]*\$100[^"]*"/g)||[]));
    ok('★★ 月合計那格才帶日期（不然幾十列分不出誰是誰）',
       /title="4日 團體課　\$5,000"/.test(out));
    ok('★★★ 定義那句只留在表頭，格子裡不再重複',
       !/<td[^>]*title="團課收入：/.test(out) && /<th[^>]*title="團課收入：/.test(out));
    ok('★★★ 沒有會員姓名、沒有合計列（使用者說多餘的那三樣）',
       !/title="[^"]*・M4[^"]*"/.test(out) && !/title="[^"]*合計 \$/.test(out));
