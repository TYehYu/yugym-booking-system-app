...
console.log('\n④ 名單視窗：一位使用人一列，挑票用圓形卡');
{
  ...
  ok('★★ 說明文字跟著改（拆列、圓形卡挑票、沒票不列）—— 畫面改了字沒改最容易誤導',
     /* 2026-09-11 使用者：「不要規則一大堆」—— 每條縮成短句、兩張名單視窗同一份文字；
        「多張票點圓形卡挑」畫面上看得到，拿掉。要守的是「字跟畫面一致、舊的誤導句不回來」。 */
     (src.match(/<li>只列<b>有團體課票券<\/b>的會員<\/li>/g)||[]).length===2
     && (src.match(/<li>設了使用人的票<b>分開一列<\/b>，點哪列就替誰報名<\/li>/g)||[]).length===2
