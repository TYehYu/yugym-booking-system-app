...
1. **「VIP 教練課」「限定教練課」的資料歸屬**：使用者已明確表示這兩者「應該是**方案**」，不是票種
   —— 即它們被建在 `ticket_types` 裡，但語意上屬於 `course_plans`（依票券規格 V4：方案＝販售商品、
   票種＝類型）。目前有 67 張票券掛在這兩筆錯置資料上（VIP 46、限定 21）。這是**資料模型遷移**，
   不是改欄位，牽涉歷史 `member_tickets.ticket_type_id` 指向，**建議與票券系統 V4 一起做**
   （見 `docs/TICKET_SYSTEM_V4_SPEC.md`）。
   - 短期補丁：程式已修得讓它們不再造成無教練預約（§1.5 第 1 點）；但這兩筆的
     `requires_coach` 仍是 false，語意上是筆誤，可先各自改為 true 止血。
