...
推進票券 V4（落差 1 方案複製、落差 2 方案封存三態）、並啟動 Inline Expand Card 行事曆改版（階段 0 週曆7天、
階段 1 卡片標準化）。**線上版本 `260718.0942`**。今日詳情見 §1.4。
...
7. **Inline Expand Card 行事曆改版啟動**（規範 `docs/INLINE_EXPAND_CARD_SPEC.md`）：
   ...
   - 階段 1（260718.0942）：**卡片標準化** —— 內容只留時間+會員姓名；課程色上 20% 色帶 + 教練色下 80% 底；
     移除特殊卡差異；簽到改右上角 ✓ 角標。
   - **待做**：階段 2（Hover Fish-eye 放大）、階段 3（Click 原地展開取代 Modal 的查看/修改/簽到/取消）、
     階段 4（新增預約也改原地展開）。階段 3/4 取代營運核心 Modal 流程，逐階段做、保留舊流程可回退。
