...
（階段 0 週曆7天、1 卡片標準化、2 Hover Fish-eye、3a 點卡原地展開+簽到/取消+邊緣感知）。
**線上版本 `260718.1026`**。今日詳情見 §1.4。
...
7. **Inline Expand Card 行事曆改版啟動**（規範 `docs/INLINE_EXPAND_CARD_SPEC.md`）：
   ...
   - 階段 3a（260718.1026）：**Click 原地展開** —— 點卡不開 Modal，原地展開查看 + 簽到/取消（卡內兩段確認）；
     編輯暫開舊 Modal 回退；靠邊卡片加**邊緣感知**（hover 與展開都會自動往內、不超出畫面）。
   - **待做**：階段 3b（原地展開卡直接編輯課程/教練/票券/備註，取代編輯 Modal）、階段 4（新增預約也改原地展開）。
     兩者取代營運核心 Modal，逐階段做、保留舊流程可回退。
