...
-- 2026-08-03 補（使用者問系統壓力）：通知時間＝上課時間（台北 08:00–21:30），
-- 深夜清晨必為空跑 → 窗口收斂為 '0,30 0-13 * * *'（UTC）＝台北 08:00–21:30，
-- 一天 48 次降為 28 次。負載本就極小（每次一個 HTTP＋一次小查詢，
-- 對照 line-auth-warm 每 5 分鐘一次跑了一週無事），此為順手節流。
